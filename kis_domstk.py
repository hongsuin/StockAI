...
##############################################################################################
# [국내주식] 기본시세 > 국내주식기간별시세(일/주/월/년)두개의 함수를 합침. 
...
# Input: None (Option) 상세 Input값 변경이 필요한 경우 API문서 참조
# Output: DataFrame (Option) output
def get_inquire_daily_itemchartprice(itm_no_list, output_dv="1", div_code="J", inqr_strt_dt=None, inqr_end_dt=None, period_code="D", adj_prc="1", tr_cont="", FK100="", NK100="", output_file=None):  # [국내주식] 기본시세 > 국내주식기간별시세(일/주/월/년)
    ...
    for itm_no in itm_no_list:
        ...
        if output_dv == "1":
            if 'output1' in res:
                output1_data = res['output1'] if isinstance(res['output1'], list) else [res['output1']]
                current_data = pd.DataFrame(output1_data)
            else:
                current_data = pd.DataFrame()  # No data found
        else:
            if 'output2' in res:
                output2_data = res['output2'] if isinstance(res['output2'], list) else [res['output2']]
                current_data = pd.DataFrame(output2_data)
            else:
                current_data = pd.DataFrame()  # No data found

        # Print the columns and data of current_data for debugging
        print("Current Data Columns:", current_data.columns)
        print("Current Data:", current_data)

        # 필요한 컬럼만 추출하여 새로운 DataFrame 생성
        columns = ['item_code', 'market', 'date', 'close', 'open', 'low', 'high', 'volume']

        if not current_data.empty:
            # Print first few rows of the dataframe to understand the structure
            print(current_data.head())

        # Manually inspect the data and update the column names based on the actual data structure
        data = {
            'item_code': [itm_no] * len(current_data),
            'market': ['주식' if div_code == 'J' else 'ELW'] * len(current_data),
            'date': current_data['stck_bsop_date'] if 'stck_bsop_date' in current_data.columns else None,
            'close': current_data['stck_clpr'] if 'stck_clpr' in current_data.columns else None,
            'open': current_data['stck_oprc'] if 'stck_oprc' in current_data.columns else None,
            'low': current_data['stck_lwpr'] if 'stck_lwpr' in current_data.columns else None,
            'high': current_data['stck_hgpr'] if 'stck_hgpr' in current_data.columns else None,
            'volume': current_data['acml_vol'] if 'acml_vol' in current_data.columns else None,
        }
    ...
    if output_file:
        combined_dataframe.to_excel(output_file, index=False)
